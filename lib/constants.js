    ...
    let exportObj = {
        ...
        COMMAND: {
            ...
            CREATE_INIT_FLAG: (str) => {
                ...
                return `${PRE_COMMAND} echo "${str}" >> "${initFlagPath}"`;
            },

            DELETE_INIT_FLAG: () => {
                let initFlagPath = path.resolve(workingRoot, './__init');
                return `${PRE_COMMAND} rm -rf "${initFlagPath}"`;
            },
            ...
            */
            SET_DHCP: (ip) => {
                ...
                return `${PRE_COMMAND} echo -e "${commands.join('\n')}" >> "/etc/dhcpcd.conf"`;
            },
            ...
            */
            DELETE_DHCP: () => {
                let lineCnt = 5;
                return `${PRE_COMMAND} sed -i -n -e :a -e "1,${lineCnt}!{P;N;D;};N;ba" "/etc/dhcpcd.conf"`;
            },
